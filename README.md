...
https://opencorporates.com/companies/us_ma/001073307
...
Issuer: CN = SSL.com EV Code Signing Intermediate CA RSA R3, O = SSL Corp, L = Houston, S = Texas, C = US 

Subject: CN = SUSE LLC, O = SUSE LLC, L = Pleasant Grove, S = Utah, C = US

*******************************************************************************
### What product or service is this for?
*******************************************************************************
SUSE Liberty Linux, now a part of SUSE Multi-Linux Support offering, https://www.suse.com/products/suse-liberty-linux/
...
*******************************************************************************
shim-unsigned-x64-16.1-1.el9.src.rpm included with the submission.

*******************************************************************************
### What patches are being applied and why:
Mention all the external patches and build process modifications, which are used during your building process, that make your shim binary be the exact one that you posted as part of this application.
*******************************************************************************
- shim-16.1.tar.bz2 (Source tarball)
...
* October 2023 - NTFS vulnerabilities
  ...
  * CVE-2023-4692
*******************************************************************************
grub2-2.06-114.el9_7 has fixes included and grub,5 in SBAT.
...
*******************************************************************************
All the commits listed above have been applied.
...
The lockdown is forced into integrity mode when kernel is booted with Secure Boot enabled:
...
`CONFIG_LOCK_DOWN_IN_EFI_SECURE_BOOT` is also enabled in the builds.
...
*******************************************************************************
Old grub builds won't boot, limited by SBAT.
...
*******************************************************************************
As SUSE Liberty Linux has no public registry, please use the provided Dockerfile and an OS image available at
https://src.opensuse.org/jsegitz/shim-review/media/branch/sll9/sll9_os_image.tar.gz
https://src.opensuse.org/jsegitz/shim-review/media/branch/sll9/sll9_os_image.tar.gz.sha256sum
to reproduce the build.
The kernel src rpm is also to big and available at
https://src.opensuse.org/jsegitz/shim-review/raw/branch/sll9/kernel-5.14.0-611.11.1.el9_7.src.rpm

Have an OS image, shim SRPM and shim binary copied alongside Dockerfile before start, then run:

$ podman import sll9_os_image.tar.gz sll9_shim_review
$ podman build -f Dockerfile_img
...
*******************************************************************************
Shim has been updated from 15.8 to 16.1.
...
*******************************************************************************
The keys are in a custom built HSM that is in a secure environment. In this secure environment it's locked away and is accessed rarely and then by multiple people to ensure separation of duties.
...
shim.sll,3,SUSE Liberty Linux,shim,16.1.el9,security@suse.com
...
grub.sll,2,SUSE,grub2,2.06-114.el9_7,mailto:security@suse.com
...
fwupd-efi.sll9,1,SUSE Liberty Linux,fwupd,1.9.31,mail:security@suse.com


kernel-uki-virt:

linux,1,Red Hat,linux,5.14.0-611.11.1.el9_7.x86_64,mailto:secalert@redhat.com

linux.rhel,1,Red Hat,linux,5.14.0-611.11.1.el9_7.x86_64,mailto:secalert@redhat.com

linux.sll,1,SUSE Liberty Linux,linux,5.14.0-611.11.1.el9_7.x86_64,mailto:security@suse.com

kernel-uki-virt.rhel,1,Red Hat,kernel-uki-virt,5.14.0-611.11.1.el9_7.x86_64,mailto:secalert@redhat.com

kernel-uki-virt.sll,1,SUSE Liberty Linux,kernel-uki-virt,5.14.0-611.11.1.el9_7.x86_64,mailto:security@suse.com

systemd,1,The systemd Developers,systemd,252,https://systemd.io/

systemd.sll,1,SUSE Liberty Linux,systemd,252-55.el9_7.2,mailto:security@suse.com
...
all_video boot blscfg btrfs cat configfile cryptodisk echo ext2 fat font gcry_rijndael gcry_rsa gcry_serpent 
gcry_sha256 gcry_twofish gcry_whirlpool gfxmenu gfxterm gzio halt hfsplus http increment iso9660 jpeg loadenv 
loopback linux lvm luks mdraid09 mdraid1x minicmd net normal part_apple part_msdos part_gpt password_pbkdf2 
png reboot regexp search search_fs_uuid search_fs_file search_label serial sleep syslinuxcfg test tftp video 
xfs efi_netfs efifwsetup efinet lsefi lsefimmap connectefi backtrace chain usb usbserial_common usbserial_pl2303 
...
*******************************************************************************
grub2-2.06-114.el9_7.src.rpm included with this submission.
...
*******************************************************************************
Downstream kernel-5.14.0-611.11.1.el9_7 built from Red Hat's sources with an appropriate set of patches to enforce Secure Boot.
...
*******************************************************************************
Numerous contributions to shim and to the review process. 
We have employees helping out with this effort (Dennis Tseng, Nikolay Borisov)
